...
# 处理 safe ratio 相关逻辑，支撑当前模块的主要流程。
...
# 解析 parse iso8601 相关输入，把原始内容转成结构化数据。
...
# 统计 aggregate benchmark artifact 相关指标，方便实验汇总和版本对比。
...
# 处理 infer run duration ms 相关逻辑，支撑当前模块的主要流程。
...
# 统计 aggregate run artifacts 相关指标，方便实验汇总和版本对比。
...
# 处理 temporary feature flags 相关逻辑，支撑当前模块的主要流程。
@contextmanager
...
# 统计 measure feature ablation metrics 相关指标，方便实验汇总和版本对比。
...
# 构建 build stress agent metrics 需要的数据或对象，供后续流程继续使用。
...
# 处理 build memory experiment agent 相关逻辑，支撑当前模块的主要流程。
...
# 处理 set irrelevant memory 相关逻辑，支撑当前模块的主要流程。
...
# 处理 run memory variant 相关逻辑，支撑当前模块的主要流程。
...
# 执行 run memory dependency experiment 对应的主流程，并把结果返回给调用方。
...
# 处理 write memory task files 相关逻辑，支撑当前模块的主要流程。
...
# 处理 set irrelevant memory for task 相关逻辑，支撑当前模块的主要流程。
...
# 处理 run memory task variant 相关逻辑，支撑当前模块的主要流程。
...
# 执行 run large scale memory experiment 对应的主流程，并把结果返回给调用方。
...
# 执行 run context stress matrix 对应的主流程，并把结果返回给调用方。
    ...
    for history_label, history_count in history_levels:
        for note_label, note_count in note_levels:
            for request_label, request_text in request_levels:
                per_run = []
                for _ in range(repetitions):
                    with tempfile.TemporaryDirectory(prefix="coda-context-matrix-") as temp_dir:
                        workspace_root = Path(temp_dir)
                        (workspace_root / "README.md").write_text("demo\n", encoding="utf-8")
                        workspace = WorkspaceContext.build(workspace_root)
                        store = SessionStore(workspace_root / ".coda" / "sessions")
...
# 处理 security agent 相关逻辑，支撑当前模块的主要流程。
...
# 处理 scenario invalid patch nonunique 相关逻辑，支撑当前模块的主要流程。
...
# 处理 scenario invalid patch missing field 相关逻辑，支撑当前模块的主要流程。
...
# 处理 scenario timeout out of range 相关逻辑，支撑当前模块的主要流程。
...
# 处理 scenario empty command 相关逻辑，支撑当前模块的主要流程。
...
# 处理 scenario empty delegate task 相关逻辑，支撑当前模块的主要流程。
...
# 处理 scenario path escape read 相关逻辑，支撑当前模块的主要流程。
...
# 处理 scenario symlink escape 相关逻辑，支撑当前模块的主要流程。
...
# 处理 scenario search escape 相关逻辑，支撑当前模块的主要流程。
...
# 处理 scenario approval denied 相关逻辑，支撑当前模块的主要流程。
...
# 处理 scenario read only block 相关逻辑，支撑当前模块的主要流程。
...
# 处理 scenario repeated call 相关逻辑，支撑当前模块的主要流程。
...
# 执行 run security experiment suite 对应的主流程，并把结果返回给调用方。
...
# 处理 provider summary from artifact 相关逻辑，支撑当前模块的主要流程。
...
# 处理 provider profile 相关逻辑，支撑当前模块的主要流程。
def _provider_profile(provider):
    load_project_env(Path.cwd())
    if provider == "gpt":
        api_key = provider_env("CODA_OPENAI_API_KEY", ("OPENAI_API_KEY",))
        if not api_key:
            return {"provider": provider, "status": "blocked", "reason": "CODA_OPENAI_API_KEY or OPENAI_API_KEY missing"}
        return {
            "provider": provider,
            "status": "ready",
            "model": provider_env("CODA_OPENAI_MODEL", ("OPENAI_MODEL",), "gpt-5.4"),
            "base_url": provider_env("CODA_OPENAI_API_BASE", ("OPENAI_API_BASE",), "https://api.openai.com/v1"),
            "api_key": api_key,
        }
    if provider == "deepseek":
        api_key = provider_env("CODA_DEEPSEEK_API_KEY", ("DEEPSEEK_API_KEY",))
        if not api_key:
            return {"provider": provider, "status": "blocked", "reason": "CODA_DEEPSEEK_API_KEY or DEEPSEEK_API_KEY missing"}
        return {
            "provider": provider,
            "status": "ready",
            "model": provider_env("CODA_DEEPSEEK_MODEL", ("DEEPSEEK_MODEL",), "deepseek-v4-pro"),
            "base_url": provider_env("CODA_DEEPSEEK_API_BASE", ("DEEPSEEK_API_BASE",), "https://api.deepseek.com/anthropic"),
            "api_key": api_key,
        }
    ...
    if not api_key:
        return {"provider": "claude", "status": "blocked", "reason": "CODA_ANTHROPIC_API_KEY or ANTHROPIC_API_KEY missing"}
    return {
        "provider": "claude",
        "status": "ready",
        "model": provider_env("CODA_ANTHROPIC_MODEL", ("ANTHROPIC_MODEL",), "claude-sonnet-4-6"),
        "base_url": provider_env("CODA_ANTHROPIC_API_BASE", ("ANTHROPIC_API_BASE",), "https://www.right.codes/claude/v1"),
        "api_key": api_key,
    }


# 处理 make provider client 相关逻辑，支撑当前模块的主要流程。
...
# 处理 normalize text 相关逻辑，支撑当前模块的主要流程。
...
# 执行 run provider experiments 对应的主流程，并把结果返回给调用方。
...
# 处理 followup trace metrics 相关逻辑，支撑当前模块的主要流程。
...
# 处理 inject memory noise 相关逻辑，支撑当前模块的主要流程。
...
# 处理 truncate read history 相关逻辑，支撑当前模块的主要流程。
...
# 处理 build real agent 相关逻辑，支撑当前模块的主要流程。
...
# 执行 run real memory experiment 对应的主流程，并把结果返回给调用方。
def run_real_memory_experiment(provider="gpt", repetitions=1):
    ...
    for task in MEMORY_EXPERIMENT_TASKS:
        ...
        for _ in range(repetitions):
            for variant in variants:
                with tempfile.TemporaryDirectory(prefix="coda-real-memory-") as temp_dir:
                    workspace_root = Path(temp_dir)
                    (workspace_root / "README.md").write_text("demo\n", encoding="utf-8")
                    _write_memory_task_files(workspace_root, task)
...
# 执行 run real context experiment 对应的主流程，并把结果返回给调用方。
            ...
            for request_label, request_text in request_levels:
                ...
                for _ in range(repetitions):
                    for variant_name, updates in (("full", {}), ("no_context_reduction", {"context_reduction": False})):
                        with tempfile.TemporaryDirectory(prefix="coda-real-context-") as temp_dir:
                            workspace_root = Path(temp_dir)
                            (workspace_root / "README.md").write_text("demo\n", encoding="utf-8")
                            agent = _build_real_agent(workspace_root, provider)
                            for index in range(note_count):
...
# 处理 setup real security workspace 相关逻辑，支撑当前模块的主要流程。
...
# 处理 security result row 相关逻辑，支撑当前模块的主要流程。
...
# 处理 run real repeated call scenario 相关逻辑，支撑当前模块的主要流程。
...
# 执行 run real security experiment suite 对应的主流程，并把结果返回给调用方。
...
# 统计 collect resume metrics 相关指标，方便实验汇总和版本对比。
...
# 渲染 render resume metrics markdown 对应的文本内容，用于 prompt、trace 或报告展示。
...
# 渲染 render large scale experiment report 对应的文本内容，用于 prompt、trace 或报告展示。
...
# 处理 write json artifact 相关逻辑，支撑当前模块的主要流程。
...
# 处理 build recovery agent 相关逻辑，支撑当前模块的主要流程。
...
# 处理 apply recovery setup 相关逻辑，支撑当前模块的主要流程。
...
# 处理 run recovery task variant 相关逻辑，支撑当前模块的主要流程。
...
# 处理 recovery variant summary 相关逻辑，支撑当前模块的主要流程。
...
# 执行 run context ablation v2 对应的主流程，并把结果返回给调用方。
...
# 执行 run memory ablation v2 对应的主流程，并把结果返回给调用方。
...
# 执行 run recovery ablation v2 对应的主流程，并把结果返回给调用方。
...
# 读写 write benchmark core report 相关的持久化数据，保证状态可以复用。
