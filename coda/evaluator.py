...
# 澶勭悊 current locale 鐩稿叧閫昏緫锛屾敮鎾戝綋鍓嶆ā鍧楃殑涓昏娴佺▼銆?
def _current_locale():
    try:
...
# 澶勭悊 fixture snapshot id 鐩稿叧閫昏緫锛屾敮鎾戝綋鍓嶆ā鍧楃殑涓昏娴佺▼銆?
...
# 妫€鏌?validate benchmark 鐩稿叧鏉′欢鏄惁鎴愮珛锛岀敤浜庢彁鍓嶆嫤鎴紓甯告儏鍐点€?
...
# 璇诲啓 load benchmark 鐩稿叧鐨勬寔涔呭寲鏁版嵁锛屼繚璇佺姸鎬佸彲浠ュ鐢ㄣ€?
...
# 缁熻 summarize rows 鐩稿叧鎸囨爣锛屾柟渚垮疄楠屾眹鎬诲拰鐗堟湰瀵规瘮銆?
...
# 澶勭悊 checkpoint payload 鐩稿叧閫昏緫锛屾敮鎾戝綋鍓嶆ā鍧楃殑涓昏娴佺▼銆?
...
# 澶勭悊 apply task setup 鐩稿叧閫昏緫锛屾敮鎾戝綋鍓嶆ā鍧楃殑涓昏娴佺▼銆?
...
class BenchmarkEvaluator:
    ...
    def __init__(
        ...
        self.workspace_root = Path(workspace_root) if workspace_root is not None else Path(
            tempfile.mkdtemp(prefix="coda-benchmark-")
        )
    ...
    # 澶勭悊 load 鐩稿叧閫昏緫锛屾敮鎾戝綋鍓嶆ā鍧楃殑涓昏娴佺▼銆?
    def load(self):
        return load_benchmark(self.benchmark_path, repo_root=self.repo_root)

    # 澶勭悊 run 鐩稿叧閫昏緫锛屾敮鎾戝綋鍓嶆ā鍧楃殑涓昏娴佺▼銆?
    def run(self):
        benchmark = self.load()
    ...
    # 鎵ц run task 瀵瑰簲鐨勪富娴佺▼锛屽苟鎶婄粨鏋滆繑鍥炵粰璋冪敤鏂广€?
    ...
    # 澶勭悊 failure category 鐩稿叧閫昏緫锛屾敮鎾戝綋鍓嶆ā鍧楃殑涓昏娴佺▼銆?
    ...
    # 澶勭悊 write artifact 鐩稿叧閫昏緫锛屾敮鎾戝綋鍓嶆ā鍧楃殑涓昏娴佺▼銆?
    def _write_artifact(self, artifact):
        self.artifact_path.parent.mkdir(parents=True, exist_ok=True)
        self.artifact_path.write_text(json.dumps(artifact, indent=2, sort_keys=True) + "\n", encoding="utf-8")


# 澶勭悊 digest file 鐩稿叧閫昏緫锛屾敮鎾戝綋鍓嶆ā鍧楃殑涓昏娴佺▼銆?
def _digest_file(path):
    return "sha256:" + hashlib.sha256(Path(path).read_bytes()).hexdigest()


# 鎵ц run fixed benchmark 瀵瑰簲鐨勪富娴佺▼锛屽苟鎶婄粨鏋滆繑鍥炵粰璋冪敤鏂广€?
...
# 鎵ц run harness regression v2 瀵瑰簲鐨勪富娴佺▼锛屽苟鎶婄粨鏋滆繑鍥炵粰璋冪敤鏂广€?
