...
# 验证 run context ablation v2 writes expected artifact 场景，确保对应功能按预期工作。
...
# 验证 provider profile loads project env before reading deepseek config 场景，确保对应功能按预期工作。
def test_provider_profile_loads_project_env_before_reading_deepseek_config(tmp_path, monkeypatch):
    ...
    with patch.dict(
        os.environ,
        {
            "DEEPSEEK_API_KEY": "dummy-legacy-deepseek",
    ...
    assert profile["api_key"] == "dummy-project-deepseek"
    assert profile["model"] == "deepseek-v4-pro"
    assert profile["base_url"] == "https://api.deepseek.com/anthropic"


# 验证 run memory ablation v2 writes expected artifact 场景，确保对应功能按预期工作。
...
# 验证 run recovery ablation v2 writes expected artifact 场景，确保对应功能按预期工作。
...
# 验证 write benchmark core report marks resume safe metrics 场景，确保对应功能按预期工作。
