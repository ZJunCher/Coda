...
from coda.run_store import RunStore
from coda.task_state import STOP_REASON_FINAL_ANSWER_RETURNED, TaskState


# 验证 run store creates run directory and state file 场景，确保对应功能按预期工作。
...
# 验证 run store appends trace jsonl 场景，确保对应功能按预期工作。
...
# 验证 run store writes report json 场景，确保对应功能按预期工作。
...
# 验证 run store tolerates missing final report 场景，确保对应功能按预期工作。
