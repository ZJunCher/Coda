...
from coda.context_manager import ContextManager


# 构建 build workspace 需要的数据或对象，供后续流程继续使用。
...
# 验证 context manager assembles sections in expected order 场景，确保对应功能按预期工作。
...
# 验证 context manager reduces relevant memory before history and preserves newer context 场景，确保对应功能按预期工作。
...
# 验证 context manager renders top three episodic notes per note under budget 场景，确保对应功能按预期工作。
...
# 验证 context manager preserves current request when over budget 场景，确保对应功能按预期工作。
...
# 验证 context manager collapses older duplicate reads into one summary line 场景，确保对应功能按预期工作。
...
# 验证 context manager summarizes older tool output into one line 场景，确保对应功能按预期工作。
...
# 验证 context manager relevant memory can mix durable notes 场景，确保对应功能按预期工作。
